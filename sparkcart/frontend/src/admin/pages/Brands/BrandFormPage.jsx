...
export default function BrandFormPage() {
  ...
  useEffect(() => {
    ...
    adminBrands.get(id, controller.signal).then((payload) => { const brand = payload.data || payload; setForm({ ...initial, ...brand }); setLogoUrl(brand.logo_url || ''); setLoading(false) }).catch((nextError) => { showError('Brand Load Failed', nextError.message); setLoading(false) })
  ...
  const chooseLogo = (event) => {
    ...
    if (!file) return
    if (!['image/svg+xml', 'image/png', 'image/webp'].includes(file.type) || file.size > 2 * 1024 * 1024) showError('Invalid Logo', 'Logo must be SVG, PNG or WEBP and no larger than 2 MB.')
    else setLogo(file)
  }
  const submit = async (event) => {
    ...
    try {
      ...
      clearAdminCatalogCache(); showSuccess(id ? 'Brand Saved' : 'Brand Created', id ? 'Brand saved successfully.' : 'Brand created successfully.')
      navigate(`/admin/brands/${brand.id}/edit`, { replace: true })
      setLogoUrl(brand.logo_url || ''); setLogo(null)
    } catch (nextError) { setErrors(nextError.errors || {}); showError('Brand Save Failed', nextError.message) } finally { setSaving(false) }
  }
  if (loading) return <LoadingState />
  return <form className="admin-page" onSubmit={submit} noValidate><PageHeader eyebrow={id ? 'Brand editor' : 'New manufacturer'} title={id ? `Edit ${form.name}` : 'Add brand'} description="Control manufacturer details and how the brand appears across the storefront." actions={<><Link className="admin-button admin-button--secondary" to="/admin/brands">Cancel</Link><button className="admin-button" disabled={saving}>{saving ? 'Saving…' : 'Save Brand'}</button></>} />
    ...
    <ConfirmDialog open={removeOpen} title="Remove brand logo?" message="The current logo file will be permanently removed." destructive confirmLabel="Remove logo" onClose={() => setRemoveOpen(false)} onConfirm={async () => { setRemoveOpen(false); try { const payload = await adminBrands.removeLogo(id); setLogoUrl((payload.data || payload).logo_url || ''); showSuccess('Brand Logo Removed', 'The brand logo was removed.') } catch (nextError) { showError('Logo Removal Failed', nextError.message) } }} />
  </form>
}
