...
export default function CategoriesPage() {
  ...
  useEffect(() => {
    ...
    adminCategories.list({ per_page: 100 }, controller.signal).then((payload) => setState((current) => ({ ...current, status: 'success', items: payload.data || [] }))).catch((error) => { if (error.name !== 'AbortError') setState((current) => ({ ...current, status: 'error', error: error.message })) })
  ...
  const save = async (event) => {
    ...
    try {
      ...
      clearAdminCatalogCache(); notify(editing ? 'Category updated.' : 'Category created.')
      setEditing(null); setForm(empty); setState((current) => ({ ...current, retry: current.retry + 1 }))
    } catch (error) { setErrors(error.errors || {}); notify(error.message, 'error') }
  }
  return <div className="admin-page"><PageHeader eyebrow="Catalogue organisation" title="Categories" description="Manage customer-facing product groupings without changing existing filter URLs." />
    ...
    </div><ConfirmDialog open={Boolean(archive)} title="Archive category?" message="Categories with assigned products cannot be archived. This action will hide an empty category from the storefront." destructive confirmLabel="Archive category" onClose={() => setArchive(null)} onConfirm={async () => { const target = archive; setArchive(null); try { await adminCategories.archive(target.id); clearAdminCatalogCache(); notify('Category archived.'); setState((current) => ({ ...current, retry: current.retry + 1 })) } catch (error) { notify(error.message, 'error') } }} />
  </div>
}
